...
class DOMWatcher {
  ...
  checkFeatureNode() {
    ...
    const nodes = this.targetElement.querySelectorAll(
      `[${TARGET_FEATURES.hasAttribute}]`
    );
    let found = false;
    ...
    nodes.forEach((node) => {
      if (
        Array.from(node.classList).some((c) =>
          c.startsWith(TARGET_FEATURES.classStartsWith)
        )
      ) {
  ...
  isPluginElement(element) {
    // 检查元素是否有插件相关的标识
    const hasFloweditClass = Array.from(element.classList || []).some(
      (className) => className.startsWith("flowedit")
    );
  ...
  async reinitializePlugins() {
    try {
      ...
      if (results.failed.length > 0) {
        console.warn(
          `[DOMWatcher] 重新初始化失败: ${results.failed.join(", ")}`
        );
      }
    } catch (error) {
