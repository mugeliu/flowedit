...
class HtmlParser {
  constructor(customTemplates = {}) {
    // 合并自定义模板和默认模板
    this.templates = this.mergeTemplates(HTML_TEMPLATES, customTemplates);
  }

  /**
   * 合并模板配置
   * @param {Object} defaultTemplates 默认模板
   * @param {Object} customTemplates 自定义模板
   * @returns {Object} 合并后的模板配置
   */
  mergeTemplates(defaultTemplates, customTemplates) {
    const merged = { ...defaultTemplates };
    
    for (const [blockType, templates] of Object.entries(customTemplates)) {
      if (merged[blockType]) {
        merged[blockType] = { ...merged[blockType], ...templates };
      } else {
        merged[blockType] = templates;
      }
    }
  ...
  renderTemplate(template, data, blockType) {
    ...
    switch (blockType) {
      ...
      case 'paragraph':
        // 对于标题和段落，处理内联样式而不是转义
        const processedContent = this.sanitizeHtml(this.processInlineStyles(data.text || ''));
        rendered = rendered.replace(/{{content}}/g, processedContent);
        break;
      
      case 'quote':
        // 引用块也需要处理内联样式
        const processedQuoteContent = this.sanitizeHtml(this.processInlineStyles(data.text || ''));
        rendered = rendered.replace(/{{content}}/g, processedQuoteContent);
        const caption = data.caption ? `<cite style="${this.templates.inlineStyles.cite}">${this.sanitizeHtml(this.processInlineStyles(data.caption))}</cite>` : '';
      ...
      case 'code':
        ...
        rendered = rendered.replace(/{{content}}/g, this.escapeHtml(data.code || ''));
        break;
      ...
      default:
        // 通用内容替换，处理内联样式
        const defaultContent = this.sanitizeHtml(this.processInlineStyles(data.text || data.content || ''));
  ...
  processInlineStyles(text) {
    ...
    Object.entries(styleEnhancements).forEach(([tag, replacement]) => {
      ...
      if (!replacement.includes('style=') || !processedText.includes(tag)) {
        processedText = processedText.replace(new RegExp(tag.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), replacement);
      }
    });
  ...
  processLinkTags(text) {
    ...
    // 匹配 <a href="..."> 标签并添加样式
    return text.replace(/<a\s+href=["']([^"']+)["']([^>]*)>/g, (match, href, otherAttrs) => {
      // 检查是否已经有style属性
